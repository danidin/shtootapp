...
cleanup() {
  ...
  if [[ -n "$CURRENT_SESSION" ]]; then
    ...
    "$SCRIPT_DIR/oci_create_session.py" delete "$CURRENT_SESSION" 2>/dev/null || true
    CURRENT_SESSION=""
  fi
}
trap cleanup EXIT
...
create_session() {
  ...
  local out
  out=$("$SCRIPT_DIR/oci_create_session.py" create "$BASTION_OCID" "$target_ip" "$label") \
  ...
  CURRENT_SESSION="$out"
  echo "  Session OCID: $CURRENT_SESSION" >&2

  echo "  Waiting for session to become ACTIVE..." >&2
  local state attempts=0
  while true; do
    sleep 5
    state=$("$SCRIPT_DIR/oci_create_session.py" get "$CURRENT_SESSION")
    echo "    state: $state" >&2
...
deploy_ozen() {
  echo "==> Deploying ozen..."
  local session
  session=$(create_session "$OZEN_IP" "ozen")
  open_tunnel "$session" "$OZEN_IP"
...
deploy_partzoof() {
  ...
  local session
  session=$(create_session "$PARTZOOF_IP" "partzoof")
  open_tunnel "$session" "$PARTZOOF_IP"
